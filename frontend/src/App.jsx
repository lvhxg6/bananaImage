...
import { styleTransfer, textToImage, getConfig, healthCheck } from './services/api';
...
function App() {
  ...
  // 执行文生图
  const handleTextToImage = async (prompt, params) => {
    setLoading(true);
    setError(null);
    setResult(null);

    try {
      const response = await textToImage(prompt, params);

      if (response.success) {
        setResult({ ...response, mode: 'text-to-image' });
      } else {
  ...
  const handleStyleTransfer = async () => {
    ...
    try {
      ...
      if (response.success) {
        setResult({ ...response, mode: 'style-transfer' });
      } else {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                基于 Gemini 3 Pro Image 的 AI 图片生成工具
              </p>
            </div>
            {serverStatus && (
            ...
            {result ? (
              <div className="space-y-6">
                ...
                <div className="text-center">
                  <button
                    onClick={activeMode === 'style-transfer' ? handleFullReset : handleReset}
                    className="text-primary-600 hover:text-primary-700 text-sm"
                  >
                    {activeMode === 'text-to-image' ? '继续生成' : '开始新的风格迁移'}
                  </button>
                </div>
              </div>
            ) : (
                    ...
                    <div>
                      ...
                      <div className="mt-6 bg-blue-50 rounded-lg p-4">
                        ...
                        <ul className="text-sm text-blue-700 space-y-1">
                          <li>1. 上传一张内容图片（保留其结构）</li>
                          <li>2. 上传一张风格图片（应用其风格）</li>
                          <li>3. 选择输出参数</li>
                          <li>4. 点击"开始风格迁移"</li>
                          <li>5. 等待 15-30 秒获取结果</li>
                        </ul>
                      </div>
                    </div>
                  </div>
